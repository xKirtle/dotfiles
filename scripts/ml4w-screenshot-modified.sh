...
# Quick instant mode: area selection + copy & save
take_instant_area_copy_save() {
    local pid_picker region
    ...
    if grim -g "$region" "$NAME"; then
        ...
        if command -v wl-copy >/dev/null 2>&1; then
            wl-copy -t image/png < "$HOME/$NAME"
        fi
        notify-send -t 1000 "Screenshot saved & copied to clipboard: $screenshot_folder/$NAME"
        [[ -f "$HOME/$NAME" && -d "$screenshot_folder" && -w "$screenshot_folder" ]] && mv "$HOME/$NAME" "$screenshot_folder/"
    fi
}
...
if [[ "$1" == "--instant" ]]; then
    ...
    take_instant_area
    exit 0
elif [[ "$1" == "--instant-area-copysave" ]]; then
    take_instant_area_copy_save
    exit 0
fi
